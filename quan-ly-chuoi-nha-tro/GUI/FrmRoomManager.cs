...
namespace quan_ly_chuoi_nha_tro.GUI
{
    public class FrmRoomManager : Form
    {
        ...
        private async System.Threading.Tasks.Task LoadDataAsync()
        {
            try
            {
                _table = await _bll.GetRoomsAsync();
        ...
        private void ApplyFilter()
        {
            ...
            else
            {
                ...
                if (_table.Columns.Contains("RoomNumber"))
                    filters.Add($"RoomNumber LIKE '%{keyword}%'");
                if (_table.Columns.Contains("RoomId"))
                    filters.Add($"Convert(RoomId, 'System.String') LIKE '%{keyword}%'");
                if (_table.Columns.Contains("BranchId"))
                    filters.Add($"Convert(BranchId, 'System.String') LIKE '%{keyword}%'");

                _table.DefaultView.RowFilter = filters.Count > 0
        ...
        private async System.Threading.Tasks.Task DeleteSelectedAsync()
        {
            ...
            int id = FrmRoomEditor.ReadInt(row, 0, "RoomId", "RoomID", "Id");
            if (id <= 0)
            {
                MessageBox.Show("Không xác định được RoomId để xóa.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            string number = FrmRoomEditor.ReadString(row, "RoomNumber", "RoomCode") ?? id.ToString();
