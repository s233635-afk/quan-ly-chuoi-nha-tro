...
namespace quan_ly_chuoi_nha_tro.GUI
{
    public class FrmTenantManager : Form
    {
        ...
        private async System.Threading.Tasks.Task LoadDataAsync()
        {
            try
            {
                _table = await _bll.GetTenantsAsync();
                _grid.DataSource = _table;
                _lblCount.Text = $"Tổng: {_table.Rows.Count}";
            }
            catch (Exception ex)
            {
                MessageBox.Show("Lỗi tải khách thuê: " + ex.Message, "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void ApplyFilter()
        {
            ...
            else
            {
                _table.DefaultView.RowFilter =
                    $"FullName LIKE '%{keyword}%' OR PhoneNumber LIKE '%{keyword}%' OR IdentityCard LIKE '%{keyword}%'";
            }
        ...
        private async System.Threading.Tasks.Task DeleteSelectedAsync()
        {
            ...
            int id = Convert.ToInt32(row["TenantId"]);
            if (MessageBox.Show($"Xóa khách thuê ID {id}?", "Xác nhận", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
